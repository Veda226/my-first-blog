    ...
    if request.method == 'POST':
        ...
        if isbn == "":
            ...
            with open('U:\\django_test\\mysitetest\\blog\\templates\\blog\ISBN2.html', 'w') as myfile:
                ...
                myfile.write(
                    "<th class=""col-md-1"">" + "  " + "</th><th class=""col-md-1"">Jan</th> <th>Feb</th> <th>Mar</th> <th>Apr</th> <th>May</th> <th>Jun</th> <th>Jul</th> <th>Aug</th> <th>Sep</th> <th>Oct</th> <th>Nov</th> <th>Dec</th></tr> </thead> <tbody> <tr> ")
                with open('C:\\Users\\prasadv\\Desktop\\Stock Opt\\Clustering output file\ISBN_Data_SOMCluster_Period_nrm.csv','rt') as f:
                    reader = csv.reader(f, delimiter=',')
                    for row in reader:
                        for field in row:
                            if field == username:
                                lista = row[32:-1]  # lista is the range starting from p1 to p#
                                max_num = max(list(map(int, row[32:-1])))
                                print(max_num)
                                listb = [int(i[0]) for i in row[8:9]]  # taking only the month from the list row
                                ...
                                p = figure(plot_width=1500, title=username+'- (Current Edition)', tools="",x_range=source.data["weight"],y_range=ranges.Range1d(start=0, end=(max_num + 100)), toolbar_location=None)
                                p.line(months, Part_for_chart, line_width=2)
                                labels = LabelSet(x='weight', y='height', text='height', level='glyph', x_offset=-13.5,y_offset=0, source=source, text_font_size="10pt", render_mode='canvas')
