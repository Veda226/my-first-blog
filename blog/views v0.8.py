...
def SalesData(request):
                ...
                with open('C:\\Users\\prasadv\\Desktop\\Stock Opt\\Clustering output file\Consolidation_meta.csv','rt') as f:
                    ...
                    for i in range(0,2):
                        if i == 2:
                            ...
                            GG = GG_combined
                        if i <2:
                            GG = df[(df['ISBN'] == username) & (df['SUPPLY SITE'] == warehouses[i])]                     #searching the ISBN in the dataframe
                            edition = 2
                            str_Html_L_R = test(GG,edition,"Current Edition-")
    ...
    ###############################################################################################################################################################################################                        
                            if previous_isbn:
                                ...
                                if not GG1.empty:
                                    ...
                                    str_Html_previous = test(GG1,edition,"Previous Edition-")
    ...
    ###############################################################################################################################################################################################
                            if previous_isbn_1:
                                ...
                                if not GG2.empty:
                                    ...
                                    str_Html_previous = test(GG2,edition,"Previous Edition-")
    ...
    onclick="window.location='Combined';">Combined</td></form></tr>"""
                            ...
                            #str_html_temp=("""<div class="panel panel-default"><div id="collapse3" class="collapseOne panel-collapse collapse"><div class="panel-body"><br/><div class="bs-example" data-example-id="table table-bordered" id ="table table-bordered"><table class= "table table-bordered"> <thead> <th class=""col-md-1"">""" + "Year" + """</th><th class=""col-md-1"">Jan</th> <th>Feb</th> <th>Mar</th> <th>Apr</th> <th>May</th> <th>Jun</th> <th>Jul</th> <th>Aug</th> <th>Sep</th> <th>Oct</th> <th>Nov</th> <th>Dec</th></tr> </thead></table></div></div></div></div>""")
                            
                            #myfile.write("""<div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" href="#collapse2">Current Edition</a></h4></div>"""+ str_html_temp +"""<div id="collapse2" class="panel-collapse collapse"><div class="panel-body"><br/><div class="bs-example" data-example-id="table table-bordered" id ="table table-bordered"><table class= "table table-bordered"> <thead> <th class=""col-md-1"">""" + "Year" + """</th><th class=""col-md-1"">Jan</th> <th>Feb</th> <th>Mar</th> <th>Apr</th> <th>May</th> <th>Jun</th> <th>Jul</th> <th>Aug</th> <th>Sep</th> <th>Oct</th> <th>Nov</th> <th>Dec</th></tr> </thead> <tbody> <tr> """)
                        ...
                        #     #print(listc)
                        #     date1 = '1/'+listc                                          # this is used for creating a running date period for sales table[]
                        #     list_temp = re.split("/",listc)                             # spliting the date field to get year
                        #     listc= list_temp[1] 
                        #     listc = int(listc)
                        ...
                        #     period_range = pd.date_range(start=date1, end= date2, freq='MS')
                        #     #print(period_range)
                        #     while date1 < date2:
                        ...
                        #     save(p)
                        #     myfile.write(
                        #         """<table class="inlineTable"><div id="chart" >{% include "svg/bar.svg" %}</div></table></div></div></div>""")
                            if not GG1.empty:
...
def test(a,Ed,edition_curr_prev):
    GG = a
    #print(GG)
    #html_str = """{% extends 'blog/base.html' %}{% block test_to_extend_"""+ str(Ed) +""" %}"""
   ...
   # myfile.write(html_str)
    ...
    str1 = [item for sublist in str1 for item in sublist ]      #reitering the convertion from dataframe to list
    # writing the HTML tages to ISBN2 file, these html tags are for meta data, with collapsable window
    str_html_previous = ("""<div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" href="#collapse""" + str(Ed) + """">"""+ edition_curr_prev + str(Ed) +"""</a></h4></div><div id="collapse""" + str(Ed) + """" class="panel-collapse collapse"><div class="panel-body"><br/><div class="bs-example" data-example-id="table table-bordered" id ="table table-bordered"><table class= "table table-bordered"> <thead> <th class=""col-md-1"">""" + "Year" + """</th><th class=""col-md-1"">Jan</th> <th>Feb</th> <th>Mar</th> <th>Apr</th> <th>May</th> <th>Jun</th> <th>Jul</th> <th>Aug</th> <th>Sep</th> <th>Oct</th> <th>Nov</th> <th>Dec</th></tr> </thead> <tbody> <tr> """)
    lista = GG.filter(regex=("Period*"))                       # using regex, getting all period sales
    #print(lista)
    lista = lista.values.tolist()                               # removing the headers from the dataframe
    lista = [item for sublist in lista for item in sublist ]    # converting dataframe to list
...
# below while loop will generate consecutive # of months and years which is stored into months variable
    ...
    p = figure(plot_width=1500, title=str(str1[0])+'- (previous Edition)', tools="",
               x_range=source.data["weight"],
               y_range=ranges.Range1d(start=0, end=(max_num + 100)), toolbar_location=None)
    p.line(months, Part_for_chart, line_width=2)
    labels = LabelSet(x='weight', y='height', text='height', level='glyph', x_offset=-13.5,
