        ...
        if isbn == "":
            ...
            with open('U:\\django_test\\mysitetest\\blog\\templates\\blog\ISBN2.html', 'w') as myfile:
                ...
                with open('C:\\Users\\prasadv\\Desktop\\Stock Opt\\Clustering output file\Consolidation_meta.csv','rt') as f:
                    ...
                    save(p)
                myfile.write(
                    """<table class="inlineTable"><div id="chart" >{% include "svg/bar.svg" %}</div></table><div class="panel-footer">@Copyright Elsevier 2017. Created by v.prasad@elsevier.com</div>{% block test %}{% endblock %}</div></div></div></div></div></div>""")
